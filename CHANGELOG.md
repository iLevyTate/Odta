...
Root-cause fixes for the two headline complaints: **generative Ask "Couldn't parse a valid plan"** and **timer chimes / notifications going silent when the app is backgrounded**. Details with evidence in `AUDIT.md` ("v76 wave").

- **Fix (Ask / parsing)**: `parseOpsJson` now recovers ops from every reply shape the small on-device models actually produce instead of throwing: arrays truncated by `max_new_tokens` (every complete op before the cut is kept), a bare `{…}` op or an `{"ops":[…]}` wrapper, Python-style quotes / `True` / `None`, trailing commas, unquoted keys, `<tool_call>` blocks on any model, OpenAI-style `{"function":{"name","arguments"}}`, `"arguments"`/`"parameters"` in place of `"args"`, and args flattened onto the op. Ops are normalised to `{name,args}` (`mark done` → `MARK_DONE`) before validation; nothing is ever invented — `validateOps` remains authoritative.
- **Fix (Ask / retry loop)**: an unparseable reply used to make the pipeline silently re-send the *identical* prompt up to four times (reproducing the same broken output and burning the timeout). It now echoes the reply back with one corrective instruction, then falls through to the write-only retry (for commands) or a grounded prose pass (for everything else) — "Couldn't parse" is reserved for the case where the model produced nothing usable at all.
- **Fix (Ask / dates)**: the few-shot examples taught literal `<tomorrow>` / `<friday>` placeholders, which 135M–360M models copy verbatim and the validator then silently dropped (task created, date lost). Examples and the user prompt now carry concrete ISO dates, and the validator resolves the common relative words (`today`, `tomorrow`, `next monday`, `+7d`, `in 2 weeks`, `tomorrow at 9am`) as a backstop.
- **Feature (Ask / overdue)**: `QUERY_TASKS` gained `overdue`, `dueBefore`, `dueAfter`, `status`, `priority`, `tag`, `listId`, `includeDone`, `includeArchived` — "clean up my overdue tasks" now has a real tool path (the only filter before was a name substring, which "overdue" never matches). Imperative detection also learned clean-up verbs (clean/clear/organise/sort/postpone/…) and politeness prefixes ("please add…", "can you make…").
- **Fix (Ask / decoding & timeout)**: structured-output turns decode greedily (sampling at T=0.1–0.2 is where half-JSON came from); the `timeoutSec` budget is now an *idle* watchdog (abort when tokens stop arriving, 2× for prefill, hard cap) instead of a wall clock over the whole multi-turn run, so a long prompt prefilling on WASM on a phone isn't killed before its first token. Broken JSON fragments are no longer shown to the user as a prose "answer".
- **Fix (background audio — root cause)**: the keepalive oscillator ran at gain 0.0001 (≈ −80 dBFS), below Chrome's audibility threshold of −72.25 dBFS (amplitude 1/4096). The browser therefore classified the tab as *silent*: no media session, normal background throttling / page freezing, `AudioContext` suspended on screen-lock — and every pre-scheduled chime vanished. The keepalive now runs at 0.004 (≈ −51 dBFS, 20 Hz), which Chrome counts as audio playback yet is inaudible on real hardware. The Settings status line notes the tab will show a speaker icon while a timer runs.
- **Fix (wake reconciliation)**: when the audio clock had stalled while hidden (iOS always; Android/desktop whenever the context was suspended), the completion path assumed the scheduled chime had already played and skipped its fallback, while the stale nodes fired minutes late on resume. `audio.js` now snapshots wall vs audio clock on hide and measures the gap on show *before* resuming; `_reconcileTimerAfterWake` drops stale nodes, runs catch-up ticks for the Pomodoro, every quick timer and the stopwatch (playing any overdue chime + notification now), and re-schedules whatever is still running against the fresh clock. Quick timers and the stopwatch were previously not reconciled on wake at all.
- **Tests**: 652 → 684 — tolerant parser shapes, op normalisation, relative-date coercion, `QUERY_TASKS` filters, corrective-retry contract, write-retry-on-parse-failure, prose scrubbing, greedy decode, idle watchdog, keepalive threshold, audio-clock stall maths, visibility handler ordering, wake reconcile sequencing, chime-guard sweep.
- Service worker cache rotated to `odtaulai-v76`.

Second pass (same branch): three parallel audits — notification path, generative stack, timer/background layer — each fixed in place.

- **Fix (reminders)**: tapping a reminder while the app is closed now opens the task (the notification carries `./?tab=tasks&task=<id>`, and boot routes it). A backlog of stale due-date reminders no longer detonates as a wall of sticky "Missed:" alerts: implicit due-date reminders older than a day are consumed silently, more than three at once collapse into one summary notification, and a reminder is delivered by chime when notifications are off (previously it was consumed with nothing shown). A malformed `remindAt` is cleared instead of warning every 30 s forever.
- **Fix (quick add)**: a bare clock earlier than now ("standup 09:30" typed at 15:00) means tomorrow, not a reminder that fires as "Missed:" thirty seconds later.
- **Fix (settings)**: the due-date reminder opt-out (`cfg.dueNotify`) finally has a toggle; `cfg.notif === undefined` (restored backups, synced peers) no longer renders the toggle ON while silently suppressing every notification; a synced cfg updates the Settings switches; the permission diagnostic renders on a fresh install and refreshes when Settings opens; quick timers don't prompt for permission when notifications are off.
- **Fix (reminders, edge writers)**: the month-calendar drag-drop and CSV import re-arm `reminderFired` when they move a due date; timer notifications carry distinct tags so two finishing together don't collapse into one.
- **Fix (timers on relaunch)**: a quick timer restored after the PWA was killed gets its chimes rescheduled and the keepalive started (it used to finish silently the moment the tab was hidden); one that ran out while closed now gets its chime, notification and session-log entry; the linked task keeps earning credit after a reload mid-focus; state is saved synchronously on hide and on `pagehide`.
- **Fix (chimes past the lookahead)**: stopwatch and quick-timer interval chimes went permanently silent after the scheduling horizon (1 h / 90 min) because played nodes suppressed the runtime fallback — both now re-arm at the horizon.
- **Fix (battery)**: the keepalive tone, wake lock and background worker are released when a timer completes, is skipped or reset (previously only on pause); a wake-lock request racing a stop no longer leaks the lock; the AudioContext gesture primer stays armed so iOS re-suspensions recover on the next tap; lock-screen media controls act on the timer that is actually running and never start a Pomodoro by themselves.
- **Fix (Ask / loading)**: a WASM model load can now be cancelled and is watchdogged (90 s without progress), so Settings can't sit on "Loading…" forever; cancelling a download reads "Download cancelled." instead of a red `LOAD_ABORTED`; the download bar no longer jumps and freezes when a large shard joins; the post-crash re-import of the ONNX runtime is real (cache-busted) instead of a no-op.
- **Fix (Ask / correctness)**: opening the command palette no longer aborts unrelated LLM work (e.g. a running "Break down"); `SET_RECUR` without a valid value is rejected instead of wiping the recurrence; `RESCHEDULE` re-arms the reminder only when the reminder time actually moves; the destructive-apply confirm keeps its red "Apply" chrome; rejected-op rows show the op name instead of `[object Object]`; one submit shares a single time budget across the ops turns, write retry and prose pass; the main-thread fallback has the same abort watchdog as the worker; a malformed calendar feed can't throw a raw error into the chat; calendar reads expose `feedId`/`eventUid`/`eventDate` so `CREATE_FROM_EVENT` is satisfiable; streaming no longer rebuilds the conversation DOM on every token.
- **Tests**: 684 → 702.

## v75 — 2026-07-20

Bug-fix wave from a fresh three-pass review (data layer, UI/PWA layer, AI/intel layer) targeting what the v74 audit missed. Details with file:line evidence in `AUDIT.md` ("v75 follow-up wave").

- **Fix (sync / data loss)**: the save-time change detector now compares `checklists` (named checklists), `completionNote`, `hiddenUntil` (snooze), and `valuesNote`. Edits touching only those fields never bumped `lastModified`, so last-write-wins merge (P2P sync or cross-tab) silently discarded them — e.g. checking off named-checklist items on one device could be reverted by another device's older copy.
- **Fix (recurrence)**: monthly recurrence no longer skips short months when the due day is the 29th–31st. `Jan 31 → monthly` previously jumped to Mar 31 (the JS date overflowed through "Feb 31" before clamping); it now lands on Feb 28/29 as intended.
- **Fix (migration)**: `SCHEMA_VERSION` bumped to 9 to gate the default-Lists backfill added in the v74 wave. Without the bump the backfill re-ran on every load, resurrecting default lists ("Work", "Finance", …) the user had deliberately deleted.
- **Fix (reminders)**: cycling a recurring task re-arms its reminder. `reminderFired` was never reset on habit completion, so a recurring task's reminder only ever fired once, on the first cycle.
- **Fix (PWA)**: first-time visitors no longer get yanked through a full-page reload moments after first paint. The `controllerchange` reload now only fires when an old service worker is actually being replaced (there's no prior controller on a fresh install — `clients.claim()` was triggering the reload).
- **Fix (Ask / lists & reminders)**: the system prompt's few-shot examples taught two argument forms the validator silently drops — `listName` (list assignment/moves never worked via Ask; examples now use `listId` and `CHANGE_LIST`) and `remindAt` on `CREATE_TASK` (the "remind me to X" flow created the task but dropped the reminder; `CREATE_TASK` now accepts and applies `remindAt`). A new test pins every prompt example's args to the schema so this class can't regress.
- **Security (Ask / prompt injection)**: closed the residual documented in the v74 audit — the 7-day calendar digest embedded in every Ask turn's base prompt is externally-authored (subscribed ICS feeds), so any turn whose prompt contains it is now tainted from the start and its write ops always land in review instead of auto-applying, even if the model never calls `GET_CALENDAR_EVENTS`.
- **Fix (intel)**: the "possible duplicate" badge no longer scores live tasks against archived tasks or orphaned embeddings of deleted tasks (parity with the duplicate-pairs view, which already filtered them).
- **UX (shortcuts)**: pressing 1–5 now actually switches top-level tabs — the cheat-sheet and command-palette advertised the shortcut but no handler existed. Cmd/Ctrl+N no longer hijacks the browser's new-window shortcut while typing in a textarea, select, or contenteditable (the guard only covered `<input>`). Panel enter-animations are now suppressed on repeat visits for programmatic tab switches too, not just clicks (the old `window.showTab` wrapper was bypassed by internal callers).
- **Fix (CSP / settings)**: the life-area color dots in the classification manager actually show their color now. They were rendered as inline `style="background:…"` attributes, which the strict CSP (`style-src 'self'`) silently blocks — colorless dots plus a console violation per dot per render. Colors are applied via the CSSOM after insert, and a new test sweeps `js/` and `index.html` to keep inline style attributes out.
- **Fix (smoke tooling)**: the browser smoke check now actually fails on CSP violations — its noise-filter entry for style-src messages never matched Chromium's real message text (and was conceptually wrong; CSSOM writes don't trigger violations). The per-tab visibility probe also read inline `display`, which the app never sets (`showTab` toggles the `hidden` attribute), so it unconditionally reported panels visible; it now checks `hidden`/`offsetParent`. With the first-install reload fix above, the smoke suite also stops crashing mid-run on fresh profiles.
- **Cleanup**: removed a dead `Math.min(500, …)` cap on Ask calendar reads (the argument coercer already clamps `limit` to 100).
Second pass on the same branch (adversarial regression review + fresh lenses on the undo layer and import/export):

- **Fix (undo / data loss)**: undoing an AI-applied delete of a task with subtasks now restores the whole subtree — the undo snapshot only captured the root, so the Undo toast brought back a childless parent and permanently lost every descendant.
- **Fix (undo)**: undoing "mark done" on a recurring task actually reverts the logged completion and the checklist reset. The snapshot was a shallow copy while the habit cycle mutates `completions[]` and checklist items in place, so the "before" state mutated along with the live task.
- **Fix (duplicate)**: AI-duplicated tasks no longer share mutable state with the source (habit log, named checklists, values alignment) — completing a cycle on either task corrupted the other's history. The copy also no longer carries the source's attachment ids (blob records are keyed by the source task, so removing an attachment from either task deleted the other's blob); a duplicate starts with no attachments and fresh history.
- **Fix (reminders, regression in this wave)**: the habit-reminder re-arm above left an explicit `remindAt` at its now-past timestamp, causing a spurious "Missed:" notification ~30 seconds after logging a cycle and blocking future due-date reminders. `remindAt` now rolls forward with the recurrence, keeping its time-of-day.
- **Fix (CSV)**: export → import round-trips to identity. The export side's formula-injection guard (leading `'` on cells starting with `= + - @`) was never stripped on import, so task names like `@home water plants` gained a literal apostrophe on every round-trip.
- **Fix (modals)**: pressing Escape to dismiss a Status/Priority/Due pill picker inside the task-detail modal no longer tears down the whole modal — both Modal and Dropdown listen for Escape in the capture phase on `document`, where `stopPropagation()` can't suppress the other; Modal now yields while a Dropdown is open.
- **Fix (notifications on file://)**: timer and reminder notifications work again when running from a `file://` snapshot. `notify()` took the ServiceWorker path whenever the API object existed — but no worker is ever registered on `file://`, so the notification promise never resolved and the main-thread fallback was unreachable.
- **Fix (URL hygiene)**: the `?openfile=1` routing param from "Open with Odta" launches is scrubbed from the address bar like the share-target params.
- **Tests**: 597 → 650 — monthly-recurrence clamping (functional), save-comparator field coverage, migration version gate (asserts `SCHEMA_VERSION` ≥ highest migration step so an ungated step can't recur), habit-cycle reminder re-arm incl. `remindAt` roll-forward (functional), prompt-example/schema consistency, first-install reload guard, shortcut integrity, duplicate-score filtering, calendar-block taint seeding, inline-style-attribute sweep, undo-snapshot integrity, CSV round-trip identity.
- Service worker cache rotated to `odtaulai-v75`.

## v74 — 2026-07-04

Full-codebase audit wave: three parallel audits (core app layer, AI/sync/network layer, infrastructure/CI) followed by fixes for every confirmed finding. Details and file:line evidence live in `AUDIT.md` ("v74 audit wave").

- **Security (sync)**: connecting to a pairing code no longer ships your entire task database the instant the data channel opens. The outbound side now sends only a lightweight `hello` and waits for the other device's user to actually click **Accept** (signalled by their first substantive message) before exchanging state — previously a mistyped code could deliver your full vault to a stranger's browser before they ever accepted. Live-edit broadcasts and the post-merge ack are gated behind the same handshake. Older (≤v73) clients still interop: their eager `state`/`patch` counts as acceptance. Also fixed on the acceptor side: wiring an already-open channel now sends state (PeerJS never replays `open` for late listeners — before the handshake change this was masked by the initiator's eager send).
- **Security (Ask auto-apply / prompt injection)**: when an Ask turn's read rounds ingest externally-authored content (calendar-feed events via `GET_CALENDAR_EVENTS` — text controlled by whoever owns a subscribed ICS feed), the resulting write batch is marked tainted and is **never auto-applied**, even in auto mode — it lands in review and the user clicking Apply is the consent. Vault-only reads (`QUERY_TASKS` etc.) keep auto-apply working. Known residual (documented in AUDIT.md): the base prompt's 7-day calendar digest is a smaller injection surface not covered by this gate.
- **Security (CSP/headers)**: dropped `https://cdn.jsdelivr.net` and `https://unpkg.com` from `script-src` — every library and model is vendored, so the allowances only widened the XSS boundary. Removed the matching dead preconnect hints. `ensureClassificationConfig` now also normalizes a synced peer's `cfg` (same allow-list the backup-import path applies).
- **Fix (calendar feeds / RRULE)**: two recurrence-expansion bugs. (1) `COUNT` was only counted against occurrences *inside* the ±180-day window, so a long-exhausted `COUNT=20` recurrence from years ago re-materialized phantom occurrences in every new window. COUNT now tracks true occurrence positions (EXDATE'd instances still don't consume slots, matching existing semantics). (2) Expansion iterated from DTSTART with a 2000-iteration cap, so a still-active daily event older than ~5.5 years burned the budget before reaching today and silently vanished — DAILY/WEEKLY rules now fast-forward arithmetically to the window edge.
- **Fix (on-device LLM)**: aborting a generation whose worker is wedged in native ONNX code (never processes the abort message) no longer hangs Ask forever with a dead Stop button. A 9s watchdog rejects the caller with the normal "Stopped." path, terminates the wedged worker, and lets the next load spawn a fresh one.
- **Fix (timer)**: `↻ Cycle` now cancels any queued auto-advance/auto-start — previously a reset issued while a phase-complete auto-start was pending (a window widened by the confirm dialog) was overridden ~300ms later by the timer starting itself. `resetPhase` already did this; `resetAll` was missed.
- **Fix (timer / cross-tab)**: starting or resuming the Pomodoro now persists state like pausing does. Beyond persistence, this marks the tab dirty so a background tab's autosave merges instead of wholesale-overwriting the running timer and form state.
- **Fix (event delegation)**: delegated `data-onfocus`/`data-onblur` handlers now register in the capture phase — focus/blur don't bubble, so the bubble-phase listeners could never fire (latent; no current markup used them).
- **Fix (PWA)**: the inline fallback service worker (used only when `sw.js` fails to register) is now stale-while-revalidate like the real one — it was pure cache-first, pinning stale assets for the lifetime of a version.
- **UX (dropdowns)**: the search filter matches substrings, not just prefixes ("fox" now finds "The quick fox"); fixed an outside-click listener leak when a dropdown was opened and closed within the same frame.
- **A11y (timer)**: quick-timer play/pause/reset/remove buttons and the interval-row remove button now carry `aria-label`s (titles alone aren't reliably announced); Pomodoro pips are keyboard-operable (`role="button"`, `tabindex`, Enter/Space) with descriptive labels.
- **CI (checker hardening)**: `check-assets.mjs` regexes are attribute-order agnostic (`<script type="module" src=…>` no longer escapes the precache check), and every referenced asset is now stat-ed on disk (model weights exempt — they're optional by design); `check-inline-handlers.mjs` now catches inline handlers containing quotes (e.g. `onclick="fn('x')"`), which previously evaded the CSP regression guard.
- **Docs**: Open Graph / Twitter preview URLs point at the live `odta.app` domain instead of the stale GitHub Pages origin.
- **Cleanup**: removed the dead legacy `settingsOpen`/`toggleSettings()` no-ops from `js/timer.js`.
- **Tests**: +24 new tests — sync handshake contract, RRULE window/COUNT regression suite, gen abort watchdog (fake-timer harness), Ask external-content taint (behavioral + static wiring guards), timer reset/persist pins, event-delegation capture-phase pin.
- Service worker cache rotated to `odtaulai-v74`.
