/**
 * Phase 1 chrome-consolidation guard.
 *
 * The Tasks screen used to stack five horizontal-scroll strips (lists, tags,
 * smart views, search, control bar) that each clipped their right-most item.
 * Those were replaced by ONE compact `.filter-bar` whose four triggers open
 * bottom sheets. This test pins:
 *   1. the filter-bar triggers exist and carry the expected data-action,
 *   2. every relocated control still lives in the DOM (so the existing
 *      filter/sort JS that reads them by id keeps working),
...
test('three option sheets exist and reuse the modal-overlay chrome', () => {
  for (const id of ['listsSheet', 'tagsSheet', 'viewSheet']) {
