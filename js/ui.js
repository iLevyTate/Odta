...
function _renderCalDayAgendaHtml(isoDate, byDate, dayOnly){
  ...
  feedEvents.slice().sort((a, b) => sortKey(a).localeCompare(sortKey(b))).forEach(ev => {
    ...
    const mk = uid && typeof createTaskFromCalEvent === 'function'
      ? `<button type="button" class="cal-agenda-mk" title="Create task from this event" aria-label="Create task from event" data-action="createTaskFromCalEvent" data-args='${escAttr(JSON.stringify([String(ev.feedId), uid, String(ev.dateISO || '')]))}'>+Task</button>`
      : '';
...
function renderCalTasks(arr, isoDate){
  ...
  if(feedEvents.length){
    ...
    html += showEvs.map(ev => {
      const uid = String(ev.uid || '');
      const mk = uid && typeof createTaskFromCalEvent === 'function'
        ? `<button type="button" class="cal-ev-mk-task" title="Create task from this event" aria-label="Create task from event" data-action="createTaskFromCalEvent" data-args='${escAttr(JSON.stringify([String(ev.feedId), uid, String(ev.dateISO || '')]))}'>+Task</button>`
        : '';
